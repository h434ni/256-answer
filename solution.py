...
def choose_codec(blk):
 ...
 for c in range(5):
  ...
  if best is None or sz<best:
   best,bc=sz,c
 return bc
L0RE=re.compile(r'^(\d+) (\d+)\.(\d+)\.(\d+)\.(\d+) "(\w+) (/[^?"]*)\?id=(\d+)"'r' (\d+) (\d+) (\d+)ms "([^/]+)/([^ ]+) \(([^)]*)\)"$')
...
def logs0_unpack(buf):
 ...
 for i in range(n):
  if i in exc:
   ...
   out.append('%d %d.%d.%d.%d "%s %s?id=%d" %s %s %dms "%s/%s (%s)"'%(ts[r],cols[0][r],cols[1][r],cols[2][r],cols[3][r],me[r],pa[r],q[r],st[r],sz[r],ms[r],g1[r],g2[r],g3[r]))
   r+=1
...
def logs1_unpack(buf):
 ...
 for i in range(n):
  if i in exc:
   ...
   out.append("%d,%d.%d.%d.%d,%s,%s,%s"%(ts[r],ip[0][r],ip[1][r],ip[2][r],ip[3][r],me[r],va[r],fl[r]))
   r+=1
 return _finish(n,exc,out,tl)
...
def xs_unmix(o):
 ...
 return z&M64
def gen_rc(num):
 u=bytearray(bytes((0).to_bytes(4,"little")))
 s={0}
 c=0
...
def gen_cl(num):
 mm={1:0}
 r=bytearray(bytes((0).to_bytes(4,"little")))
...
def a181_ve(val,n):
 num=n//4
 u=ve(val,num+1)
 return val.to_bytes(4,"little")+b"".join(x.to_bytes(4,"little")for x in u[1:num])
def a181_diff(val,n):
 num=n//4
 u=ve(val,num+2)
 d=[u[k+1]-u[k]for k in range(1,num)]
 return(-val).to_bytes(4,"little",signed=True)+b"".join(x.to_bytes(4,"little",signed=True)for x in d)
def a181_u16(val,n):
 num=(n-4)//2
 u=ve(val,num+5)
 return val.to_bytes(4,"little")+b"".join((x&65535).to_bytes(2,"little")for x in u[2:num+2])
def a181_bits(val,n):
 num=(n-4)//3
 u=ve(val,num*2+5)
 t=u[3:3+num*2+2]
...
def mt_fit_check(payload):
 ...
 B=[None]*128
 for i in range(500):
  q=(words[i]|words[i+1]<<32|words[i+MT_M]<<64|words[i+MT_M+1]<<96)
  y=words[i+MT_N]
  while q:
   b=q.bit_length()-1
   v=B[b]
 ...
 for i in range(MT_N,nw):
  if pred[i]!=words[i]:
   return None
 return B
T_RESID,T_SHA,T_LCG,T_XS,T_A181,T_CA,_UNUSED,T_MT1,T_TOC,T_REF,T_FPLAN,T_RC,T_CL,T_MT4R,T_CNST,T_CNST1,T_MFAM=range(17)
...
XT_NAMES=("raw","lane2","lane3","lane4","lane5","lane6","lane7","lane8","lane12","lane16","lane24","lane32","x1","x2","x3","x4","x6","x8","x12","x16","x32","x64","x128","x256","x512","x1024","x2048","x4096","x8192","x16384","x32768","d1","d2","d3","d4","d6","d8","d12","lane3d1","lane4d1","lane2d1","lane3x1","mpack","lane3d3","lane2d2","lane4d4","d2048","x251","d251")
...
def build_plan(data):
 ...
 L0=len(samples[0])
 def score3(b):
  return min(len(zlib.compress(b,6)),len(zlib.compress(xfwd(b,31),6)),len(zlib.compress(xfwd(b,38),6)))
...
def compress(src,dst):
 ...
 for i,c in enumerate(cs):
  ...
  if mt4_basis is None:
   mt4_basis=B
 mt1=[i for i,c in enumerate(cs)if c[0]==b"MTST"and c[2]&7==1 and tag[i]==T_RESID]
 C=None
 if mt1:
 ...
 fplan_plan={}
 for g in FPLAN_KEYS:
  ids=[i for i,c in enumerate(cs)if c[0]==g[0]and c[3]==g[1]and tag[i]==T_RESID]
 ...
 out=bytearray()
 meta=d[:80]+b"".join(c[3].to_bytes(4,"big")+c[0]+bytes((c[1],c[2]))for c in cs)
 hd=bytearray()
 ...
 for k in sorted(rg):
  ...
  subs=sorted(set(cs[i][1]for i in ids))if(len(ids)>=16 and tot>=(1<<20))else[None]
  for e in subs:
   ...
   if k[0]==b"LOGS"and e in LOGS_PK:
    ...
    md=LOGS_PK[e](txt)
    if(md is not None and LOGS_UP[e](md)==txt and len(zlib.compress(md,6))<len(zlib.compress(txt.encode("latin1"),6))):
     xchoices.append(255)
     gblocks.append(md)
     continue
   ks=pick_parts(parts)
 ...
 for g in FPLAN_KEYS:
  ids=[i for i,c in enumerate(cs)if c[0]==g[0]and c[3]==g[1]and tag[i]==T_FPLAN]
  if not ids:
   continue
  plan=fplan_plan[g]
  parts=[]
...
def decompress(src,dst):
 ...
 if has_mt4r:
  for _ in range(128):
   basis.append((int.from_bytes(hd[p:p+16],"big"),int.from_bytes(hd[p+16:p+20],"big")))
   p+=20
 has_mt1=hd[p]
 p+=1
 if has_mt1:
 ...
 for g in sorted(groups):
  ids=groups[g]
  if any(cs[i][2]&7==1 for i in ids)and any(cs[i][2]&7!=1 for i in ids):
 ...
 if mt4ids:
  ...
  for i in mt4ids:
   ...
   base+=2496
   w=mt_gen([int.from_bytes(raw[j:j+4],"big")for j in range(0,2496,4)],n//4,T)
 ...
 for i in range(nc):
  if tags[i]==T_RESID:
   rg.setdefault((cs[i][0],0 if cs[i][0]==b"LOGS"else cs[i][3]),[]).append(i)
 xi=0
 for k in sorted(rg):
  ids=rg[k]
  tot=sum(cs[i][3]for i in ids)
 ...
 for g in FPLAN_KEYS:
  ids=[i for i,c in enumerate(cs)if c[0]==g[0]and c[3]==g[1]and tags[i]==T_FPLAN]
  if not ids:
   continue
  cnt,plan=fplans[fi]
  fi+=1
  ...
  def get(j):
   if done[j]is None:
    code=plan[j]
    done[j]=raw[j]if code==255 else undelta(raw[j],get(code&31),code>>5)
   return done[j]
